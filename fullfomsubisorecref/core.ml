...
let rec eval1 ctx store t = match t with
  ...
  | TmTimesfloat(fi,t1,t2) ->
      let t1',store' = eval1 ctx store t1 in
      TmTimesfloat(fi,t1',t2) , store'
  | TmApp(_,TmUnfold(_,tyS),TmApp(_,TmFold(_,tyT),v)) when isval ctx v ->
      v, store
  | TmApp(fi1,TmFold(fi2,tyS),t2) ->  
      let t2',store' = eval1 ctx store t2 in
      TmApp(fi1, TmFold(fi2,tyS), t2') , store'
  | TmApp(fi1,TmUnfold(fi2,tyS),t2) -> 
      let t2',store' = eval1 ctx store t2 in
      TmApp(fi1, TmUnfold(fi2,tyS), t2'), store'
  | TmSucc(fi,t1) ->
      let t1',store' = eval1 ctx store t1 in
      TmSucc(fi, t1'), store'
  | TmPred(_,TmZero(_)) ->
...
and kindof ctx tyT = match tyT with
  ...
  | TySink(tyT) -> 
      ...
      KnStar
  | TyRec(tyX,tyT) ->
      let ctx' = addbinding ctx tyX (TyVarBind(maketop KnStar)) in
      if kindof ctx' tyT <> KnStar then error dummyinfo "star kind expected";
      KnStar
  | _ -> KnStar
...
let rec subtype ctx tyS tyT =
   ...
   | (TySink(tyT1),TySink(tyT2)) ->
       subtype ctx tyT2 tyT1
   | (TyRec(tyX1,tyT1),TyRec(tyX2,tyT2)) ->
       let ctx1 = addbinding ctx tyX2 (TyVarBind(maketop KnStar)) in
       let ctx2 = addbinding ctx1 tyX1 (TyVarBind(TyVar(0,ctxlength ctx1))) in
       let tyT1' = typeShiftAbove 1 1 tyT1 in
       let tyT2' = typeShift 1 tyT2 in
       subtype ctx2 tyT1' tyT2'
   | (_,_) -> 
       false
